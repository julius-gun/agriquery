...
docker run -it \
    ...
    -v /data/$(id -un)/ZeroShot:/app \
    -v /data/$(id -un)/ZeroShot/results/:/results \
	$(id -un)/llm-image:1.0
```
...
*   `-v /data/$(id -un)/ZeroShot:/app`: This is a **volume mount**. It creates a link between a directory on your host machine and a directory inside the Docker container.
    *   `-v`: option for volume mount.
    *   `/data/$(id -un)/ZeroShot`: This is the path to a directory on your **host machine**.  It's likely where your project code is located. `$(id -un)` again uses your username to construct a path.
    *   `:/app`: This is the path to a directory **inside the Docker container**. In your `Dockerfile`, you set `WORKDIR /app`, so this mounts your project code into the `/app` directory inside the container.  **Any changes you make to files in `/data/$(id -un)/ZeroShot` on your host will be reflected in `/app` inside the container, and vice versa.**
*   `-v /data/$(id -un)/ZeroShot/results/:/results`:  Another volume mount, this time for the `results` directory. This ensures that any results generated by your application inside the container (in the `/results` directory) are saved to `/data/$(id -un)/ZeroShot/results/` on your host machine, so you can access them even after the container stops.
