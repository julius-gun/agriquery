...
try:
    ...
    # Assuming project root is two levels up from the current script's directory
    # (e.g., current_script_dir = RAG/visualization/plot_scripts, project_root = RAG)
    project_root_fallback = os.path.dirname(os.path.dirname(current_script_dir))
...
# --- Imports ---
try:
    # Assuming PROJECT_ROOT is added to sys.path, allowing absolute imports from project subdirectories
    from visualization.visualization_data_extractor import extract_detailed_visualization_data
    # Assuming main_visualization.py is in the same directory as this script
    ...
    print(
        ...
        "within 'visualization' or 'plot_scripts'." # Corrected path hint
    )
...
def _abbreviate_model_name(model_name: str) -> str:
    ...
    # 1. Remove preview dates like -preview-MM-DD or -preview-MM
    name = re.sub(r'-preview-\d{2}-\d{2}$', '', name)
    name = re.sub(r'-preview-\d{2}$', '', name) # For cases like -preview-05

    # 2. Specific handling for phi3 models to retain the core part (e.g., phi3_14B, phi3_mini)
    # This captures the base model name before quantization or context window suffixes.
    if name.startswith("phi3"):
        match = re.match(r'(phi3(?:_mini|_medium|_small)?(?:_\d+B)?)', name)
        if match:
            name = match.group(1) # e.g., 'phi3_14B' from 'phi3_14B_q4_medium-128k'

    # 3. General rule: remove context window suffixes like -128k, -8k, -4k, -16k
    name = re.sub(r'-\d+k$', '', name)

    # 4. General rule: remove common ollama tag parts like ':latest', ':8b', etc.
    name = re.sub(r':\w+$', '', name)
    name = re.sub(r':\d+(\.\d+)?[bB]$', '', name) # e.g. :8b, :1.5b

    # 5. Further specific simplifications for final display names
    if "gemini-2.5-flash" in name:
        ...
        name = "Qwen3 8B"
    # Use original model_name to check for specific variants that should be simplified to "Phi3 14B"
    elif "phi3_14B" in name and ("medium" in model_name or "128k" in model_name or "4k" in model_name):
        name = "Phi3 14B"
    elif "llama3.1_8B" in name:
        name = "Llama3.1 8B"
    elif "deepseek-r1_8B" in name:
        name = "Deepseek-R1 8B"
    elif "llama3.2_3B" in name:
        name = "Llama3.2 3B"
    elif "deepseek-r1_1.5B" in name:
        name = "Deepseek-R1 1.5B"
    elif "llama3.2_1B" in name:
        name = "Llama3.2 1B"

    # Note: Underscores in model names are replaced with '\_' for LaTeX compatibility
    # later when the DataFrame index is processed, so no need to do it here.
...
def _generate_single_latex_table_string(
    ...
    language: str,
    retrieval_algorithm_or_report_type: str,
    ...
    if retrieval_algorithm_or_report_type == FULL_MANUAL_INTERNAL_ID:
        ...
        # Specific filtering for Full Manual: zeroshot at a specific noise level
        df_filtered = df_data[
    ...
    # 1. Abbreviate model names in the 'question_model' column
    df_filtered['question_model'] = df_filtered['question_model'].apply(_abbreviate_model_name)

    # 2. Create a list of unique abbreviated model names in the desired sort order
    ...
    # 3. Pivot the table using the now abbreviated 'question_model' names
    try:
        ...
        print("Relevant data for pivot (head):")
    ...
    # 4. Reorder rows (models) and select/reorder columns (metrics)
    # Reindex to include all models from the sort order, filling missing with NaN
    pivot_df = pivot_df.reindex(unique_abbreviated_model_sort_order)

    # Filter out models that are in the sort order but not present in the actual data
    ...
    # Drop rows where all metric values are NaN (e.g., models with no data)
    ...
    # Select and reorder metric columns
    ...
    # 5. Apply Bold Formatting to Max Values and String Format Numeric Cells
    for metric_col_name in final_metrics_columns:
        ...
        elif metric_col_name in df_for_latex.columns: # Handle non-numeric columns (should not happen with metrics)
             df_for_latex[metric_col_name] = df_for_latex[metric_col_name].astype(str).fillna("-")

    # 6. Sanitize Model Names in Index of df_for_latex for LaTeX display
    ...
    # 7. Rename Columns of df_for_latex for Display
    ...
    try:
        ...
        # 9. Modify LaTeX String for custom formatting
        lines = latex_string.splitlines()

        # 9a. Inject \footnotesize
        ...
        # 9b. Inject custom two-line header
        toprule_idx = -1
        for i, line in enumerate(lines):
            if r'\toprule' in line:
                toprule_idx = i
        ...
        print("DataFrame that was to be converted to LaTeX (df_for_latex head):")
...
def generate_latex_reports_by_method(
    ...
    # Each item is a tuple: (internal_id_for_logic, filename_base, param_for_generate_single_table)
    # For standard algorithms, internal_id and param are the same.
    # For Full Manual, internal_id is a unique key, param is FULL_MANUAL_INTERNAL_ID.
    ...
    for report_id, filename_base, generation_param in report_configurations:
        ...
        for lang in languages_from_config:
            table_latex_string = _generate_single_latex_table_string(
                df_data=df_data,
                language=lang,
                retrieval_algorithm_or_report_type=generation_param,
...
def main():
    ...
    # PROJECT_ROOT is available via import/execution of plot_utils logic
