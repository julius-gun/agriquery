...
class RagTester:
    ...
    def __init__(
        ...
        if self.shared_embedding_retriever:
            ...
            logging.error(
                "FATAL: RagTester must be initialized with an EmbeddingRetriever instance."
            )
            raise ValueError(
                "RagTester requires a shared_embedding_retriever to be provided during initialization."
            )
        ...
        logging.info(
            f"Target Datasets (from current config): {self.target_dataset_names}"
        )
    ...
    def _validate_config(self):
        ...
        if not self.file_extensions_to_test:
            ...
            raise ValueError(
                "Error: No 'file_extensions_to_test' found in config.json."
            )
        ...
        if self.enable_reevaluation and not self.target_dataset_names:
            logging.error(
                "Error: Reevaluation is enabled but no 'question_dataset_paths' are defined in config."
            )
            raise ValueError(
    ...
    def _initialize_evaluator(self) -> Optional[Evaluator]:
        ...
        # The _validate_config method ensures this is not None.
        # Adding an assertion here to satisfy the type checker and add runtime safety.
        if not evaluator_model_name:
            logging.critical(
                "FATAL: 'evaluator_model_name' is missing, but validation passed. This should not happen."
            )
            raise ValueError("'evaluator_model_name' is unexpectedly None.")

        evaluator_llm_type, _ = self.config_loader.get_llm_type_and_config(
        ...
        if not evaluator_llm_type:
            ...
            logging.warning(
                f"Could not determine LLM type for evaluator '{evaluator_model_name}'. Assuming '{evaluator_llm_type}'."
            )
        ...
        try:
            ...
            evaluator = Evaluator(
                evaluator_llm_connector, self.evaluation_prompt_template
            )
            logging.info(
                f"Evaluator initialized with model: {evaluator_model_name} (Type: {evaluator_llm_type})"
            )
            return evaluator
        except Exception as e:
            logging.critical(
    ...
    def _load_datasets(self) -> Tuple[Dict[str, List[Dict]], int]:
        ...
        for dataset_name, dataset_path in dataset_paths.items():
            if (
                dataset_name not in self.target_dataset_names
            ...
            if dataset:
                ...
                for q_data in dataset:
                    if (
                        ...
                        and "question" in q_data
                        and "answer" in q_data
                    ):
                        q_data["dataset"] = dataset_name  # Add dataset origin
                        valid_dataset.append(q_data)
                    else:
                        logging.warning(
                            f"Dataset '{dataset_name}' contains invalid item (not dict or missing keys): {q_data}. Skipping."
                        )
                loaded_datasets[dataset_name] = valid_dataset
                total_questions += len(valid_dataset)
                logging.info(
                    f"  Loaded dataset '{dataset_name}' with {len(valid_dataset)} valid questions."
                )
            else:
                logging.warning(
                    f"  Warning: Failed to load dataset '{dataset_name}' from {dataset_path}."
                )
    ...
    def _get_chroma_collection(
        ...
        try:
            ...
            logging.error(
                "!!! This collection is required for the current test combination (embedding, keyword, or hybrid)."
            )
            logging.error(
    ...
    def _handle_reevaluation(
        ...
        current_questions_sets = {
            name: {
                q["question"] for q in data if isinstance(q, dict) and "question" in q
            }
        ...
        for dataset_name in datasets_in_results:
            ...
            if dataset_name not in self.target_dataset_names:
                ...
                logging.info(
                    f"Dataset '{dataset_name}' found in results but not in current config. Preserving existing results; excluding from re-answering & metrics."
                )
                ...
                if "results" in dataset_content and isinstance(
                    dataset_content["results"], list
                ):
                    pruned_previous_questions_map[dataset_name] = {
                        res["question"]
                        for res in dataset_content["results"]
            ...
            logging.info(
                f"Pruning results for dataset '{dataset_name}' based on current questions..."
            )
            dataset_content = previous_results["per_dataset_details"][dataset_name]
            if "results" not in dataset_content or not isinstance(
                dataset_content["results"], list
            ):
                logging.warning(
                    f"Dataset '{dataset_name}' in results has invalid 'results' list. Treating as empty for pruning."
                )
                pruned_results = []
            else:
                results_list = dataset_content["results"]
                current_q_set = current_questions_sets.get(
                    dataset_name, set()
                )  # Should exist
                if not current_q_set:
                    logging.warning(
                        f"No questions loaded for '{dataset_name}' from current config, despite being targeted. Removing all previous results for it."
                    )
                    pruned_results = []
                else:
                    ...
                    pruned_results = [
                        ...
                        and res.get("question") in current_q_set
                    ]
                    removed_count = original_count - len(pruned_results)
                    logging.info(
                        f"  Removed {removed_count} results (questions not in current '{dataset_name}'). Kept {len(pruned_results)}."
                    )

            # Update the results in the dictionary and store pruned questions
            previous_results["per_dataset_details"][dataset_name]["results"] = (
                pruned_results
            )
            pruned_previous_questions_map[dataset_name] = {
                res["question"]
        ...
        for dataset_name, current_questions_data in self.loaded_datasets.items():
            ...
            answered_questions_in_pruned = pruned_previous_questions_map.get(
                dataset_name, set()
            )
            missing_count = 0
            for question_data in current_questions_data:  # question_data is a dict {'question': ..., 'answer': ..., 'page': ..., 'dataset': ...}
                if isinstance(question_data, dict) and "question" in question_data:
                    if question_data["question"] not in answered_questions_in_pruned:
                        questions_to_answer_list.append(
                            question_data
                        )  # Add the full dict
                        missing_count += 1
                else:
                    logging.warning(
                        f"Skipping invalid item in loaded dataset '{dataset_name}': {question_data}"
                    )
    ...
    def _run_qa_phase(
        ...
        question_llm_connector: BaseLLMConnector,
        ...
        for dataset_name, dataset_questions in datasets_to_iterate.items():
            if llm_invocation_failed:
                logging.warning(
                    f"  Skipping QA for Dataset: {dataset_name} due to previous LLM invocation failure."
                )
                continue
            ...
            for i, question_data in enumerate(dataset_questions):
                ...
                if not question or expected_answer is None:  # Check both Q & A
                    logging.warning(
                        f"Skipping entry in {q_dataset_name} due to missing question/answer: {question_data}"
                    )
                ...
                logging.info(
                    f"  Answering Q {answered_questions_count}/{total_questions_in_scope} ({q_dataset_name} {i + 1}/{len(dataset_questions)}): {question[:80]}..."
                )
            ...
            grouping_dataset_name = (
                dataset_questions[0]["dataset"] if dataset_questions else dataset_name
            )
        ...
        logging.info(
            f"--- Finished Phase 1 ({log_phase}) in {overall_qa_duration:.2f} seconds ---"
        )
    ...
    def _run_evaluation_phase(
        ...
        if not self.evaluator or not self.evaluator.evaluation_prompt_template:
            logging.warning(
                "Skipping evaluation phase: Evaluator not initialized or template missing."
            )
            return intermediate_results_by_dataset, 0.0, []
        ...
        for dataset_name, dataset_data in intermediate_results_by_dataset.items():
            ...
            logging.info(
                f"  Evaluating Dataset: {dataset_name} ({total_in_dataset} results)"
            )
            ...
            for i, intermediate_result in enumerate(dataset_data.get("results", [])):
                ...
                logging.info(
                    f"    Evaluating A {evaluated_questions_count}/{total_questions_processed_in_qa} ({dataset_name} {i + 1}/{total_in_dataset})..."
                )
                ...
                if intermediate_result.get("qa_error", False):
                    logging.info(
                        "      Skipping evaluation due to prior QA/Retrieval error."
                    )
                ...
                if (
                    evaluation_result in ["yes", "no"]
                    and not eval_error
                    and dataset_name in self.target_dataset_names
                ):
    ...
    def _calculate_and_print_metrics(
        ...
        if not final_results_list_for_metrics:
            logging.warning(
                "  No valid results from target datasets available for metrics calculation."
            )
        ...
        logging.info(
            f"  Calculating metrics based on {len(final_results_list_for_metrics)} valid results from target datasets."
        )
        try:
            overall_metrics = calculate_metrics(
                final_results_list_for_metrics
            )  # calculate_metrics uses 'dataset' key internally

            logging.info(
                "\n--- Overall Evaluation Analysis (Based on Target Datasets) ---"
            )
            for key, value in overall_metrics.items():
                if key == DATASET_METRIC_KEY:
                    continue  # Skip printing this sub-dict here
    ...
    def _calculate_dataset_success_rates(
        ...
        dataset_metrics = {}
        logging.debug(
            "Calculating dataset self-evaluation success rates (for target datasets)..."
        )
            ...
            if dataset_name in evaluated_results_by_dataset:
                ...
                for result_item in results_list:
                    ...
                    if (
                        ...
                        and result_item.get("self_evaluation") in ["yes", "no"]
                        and not result_item.get("eval_error")
                    ):
                ...
                if total_valid_for_rate > 0:
                    ...
                    logging.debug(
                        f"  Dataset '{dataset_name}' success rate: {yes_count}/{total_valid_for_rate} = {rate:.4f}"
                    )
                else:
                    dataset_metrics[dataset_name] = (
                        0.0  # Rate is 0 if no valid evaluated results
                    )
                    logging.debug(
                        f"  Dataset '{dataset_name}': 0 valid results for rate calculation, rate = 0.0"
                    )
            else:
                # Target dataset not found in results (e.g., QA failed entirely for it)
                dataset_metrics[dataset_name] = 0.0
                logging.debug(
                    f"  Target Dataset '{dataset_name}' not found in evaluated results, rate = 0.0"
                )
    ...
    def _process_single_combination(
        ...
        if self.enable_reevaluation:
            ...
            if pruned_previous_results is not None and questions_to_answer is not None:
                ...
                if not questions_to_answer:
                    ...
                    logging.info(
                        "Reevaluation: No new questions to answer. Recalculating metrics on pruned results and saving."
                    )
                    ...
                    for ds_name, ds_content in pruned_previous_results.get(
                        "per_dataset_details", {}
                    ).items():
                        if (
                            ds_name in self.target_dataset_names
                        ):  # Filter by target datasets
                            for res in ds_content.get("results", []):
                                if (
                                    isinstance(res, dict)
                                    and res.get("self_evaluation") in ["yes", "no"]
                                    and not res.get("eval_error")
                                ):
                                    all_pruned_results_for_metrics.append(res)

                    overall_metrics = self._calculate_and_print_metrics(
                        all_pruned_results_for_metrics
                    )
                    if (
                        isinstance(overall_metrics, dict)
                        and "metrics_calculation_error" not in overall_metrics
                    ):
                        # Calculate dataset rates based on all data, but only for target datasets
                        dataset_success_rates = self._calculate_dataset_success_rates(
                            pruned_previous_results["per_dataset_details"]
                        )
                        overall_metrics[DATASET_METRIC_KEY] = dataset_success_rates
                        pruned_previous_results["overall_metrics"] = overall_metrics
                    else:
                        logging.error(
                            "Metrics calculation failed during reevaluation (no new questions). Saving without updated metrics."
                        )
                        pruned_previous_results["overall_metrics"] = {
                            "error": "Metrics recalculation failed",
                            DATASET_METRIC_KEY: {},
                        }

                    # Update parameters/timing notes?
                    if "test_run_parameters" not in pruned_previous_results:
                        pruned_previous_results["test_run_parameters"] = {}
                    pruned_previous_results["test_run_parameters"]["language"] = (
                        language
                    )
                    pruned_previous_results["test_run_parameters"]["file_extension"] = (
                        extension
                    )
                    pruned_previous_results["test_run_parameters"]["file_tested"] = (
                        file_identifier
                    )
                    pruned_previous_results["test_run_parameters"][
                        "reevaluation_run_type"
                    ] = "metrics_only"
                    pruned_previous_results["test_run_parameters"][
                        "evaluator_model"
                ...
                logging.info(
                    f"Reevaluation: Proceeding to answer {len(questions_to_answer)} missing questions."
                )
            ...
            # it means reevaluation failed (e.g., file not found). Fall through to normal run.
                ...
                logging.info(
                    "Reevaluation enabled but no existing file found or load failed. Performing full run."
                )
        ...
        if run_qa_phase:
            ...
            collection = self._get_chroma_collection(
                base_collection_name, chunk_size, overlap_size
            )
            if collection is None:
                logging.warning(
                    f"Skipping combination: Failed to get required ChromaDB collection '{dynamic_collection_name}'."
                )
                return
            ...
            if retrieval_algorithm == "keyword" or retrieval_algorithm == "hybrid":
                logging.info(
                    f"{retrieval_algorithm.capitalize()} Algorithm: Indexing documents from '{collection.name}'..."
                )
                try:
                    ...
                    if results and results.get("documents"):
                        ...
                        if document_chunks:
                            if isinstance(
                                ...
                                index_method = getattr(
                                    retriever,
                                    "build_keyword_index",
                                    getattr(retriever, "build_index", None),
                                )
                                if index_method:
                                    index_method(document_chunks)
                                    logging.info(
                                        f"Index built successfully for {len(document_chunks)} documents."
                                    )
                                else:
                                    raise AttributeError(
                                        "No index building method found."
                                    )
                            else:
                                raise TypeError(
                                    f"Invalid retriever type for indexing: {type(retriever).__name__}"
                                )
                        else:
                            logging.warning(
                                f"Collection '{collection.name}' has no documents. Indexing empty."
                            )
                            if isinstance(
                                retriever, (KeywordRetriever, HybridRetriever)
                            ):  # Build empty index
                        ...
                        logging.error(
                            f"Failed to fetch documents from '{collection.name}' for indexing. Skipping."
                        )
                        return
                except Exception as e:
                    logging.error(
                        f"Error fetching/building index for {retrieval_algorithm}: {e}. Skipping.",
                        exc_info=True,
                    )
                    return
            ...
            new_intermediate_results, qa_duration, qa_count = self._run_qa_phase(
                ...
                question_llm_connector=question_llm_connector,
            ...
            newly_evaluated_results, eval_duration, new_metric_results_list = (
            ...
            if self.enable_reevaluation and pruned_previous_results:
                ...
                logging.info(
                    "Reevaluation: Merging new results with pruned previous results using overwrite..."
                )
                final_results_to_save = pruned_previous_results

                # Use dict for efficient overwrite: Convert existing results lists to dicts
                merged_details = {}
                for ds_name, ds_content in final_results_to_save.get(
                    "per_dataset_details", {}
                ).items():
                    ...
                    merged_details[ds_name] = (
                        ds_content.copy()
                    )  # Copy dataset-level keys (like duration if any)
                    merged_details[ds_name]["results"] = (
                        results_dict  # Store as dict temporarily
                    )
                ...
                for ds_name, new_content in newly_evaluated_results.items():
                    ...
                    if not new_results_list:
                        continue  # Skip if no new results for this dataset

                    if ds_name not in merged_details:
                        # Dataset is entirely new (wasn't in pruned_previous_results)
                        logging.info(
                            f"  Adding new dataset '{ds_name}' from QA results."
                        )
                        merged_details[ds_name] = (
                            new_content  # Add the whole new structure
                        )
                        ...
                        target_dict = merged_details[ds_name][
                            "results"
                        ...
                        for new_res in new_results_list:
                            if isinstance(new_res, dict) and "question" in new_res:
                                ...
                                if q_str in target_dict:
                                    updated_count += 1
                                else:
                                    added_count += 1
                                target_dict[q_str] = new_res  # *** Overwrite or add ***
                                total_new_or_updated += (
                                    1  # Count effective changes/additions
                                )
                        logging.info(
                            f"  Merged results for dataset '{ds_name}': {added_count} added, {updated_count} updated."
                        )

                # Convert results dictionaries back to lists before saving final structure
                for ds_name in merged_details:
                    if "results" in merged_details[ds_name] and isinstance(
                        merged_details[ds_name]["results"], dict
                    ):
                        merged_details[ds_name]["results"] = list(
                            merged_details[ds_name]["results"].values()
                        )

                # Update the final results structure
                final_results_to_save["per_dataset_details"] = merged_details
                logging.info(
                    f"Total new/updated results merged: {total_new_or_updated}"
                )
                ...
                if (
                    ...
                    dataset_success_rates = self._calculate_dataset_success_rates(
                        final_results_to_save["per_dataset_details"]
                    )
                    overall_metrics[DATASET_METRIC_KEY] = dataset_success_rates
                    final_results_to_save["overall_metrics"] = overall_metrics
                else:
                    logging.error(
                        "Metrics calculation failed during reevaluation merge. Saving without updated metrics."
                    )
                    final_results_to_save["overall_metrics"] = {
                        "error": "Combined metrics calculation failed",
                        DATASET_METRIC_KEY: {},
                    }

                # Update timing/parameters notes for reevaluation
                if "test_run_parameters" not in final_results_to_save:
                    final_results_to_save["test_run_parameters"] = {}
                final_results_to_save["test_run_parameters"]["language"] = language
                final_results_to_save["test_run_parameters"]["file_extension"] = (
                    extension
                )
                final_results_to_save["test_run_parameters"]["file_tested"] = (
                    file_identifier
                )
                final_results_to_save["test_run_parameters"][
                    "reevaluation_run_type"
                ...
                final_results_to_save["test_run_parameters"][
                    "reevaluation_qa_duration_seconds"
                ] = qa_duration
                final_results_to_save["test_run_parameters"][
                    "reevaluation_eval_duration_seconds"
                ] = eval_duration
                final_results_to_save["test_run_parameters"]["evaluator_model"] = (
                    self.config_loader.get_evaluator_model_name()
                )  # Update evaluator potentially

            else:
                # Normal run: Calculate metrics on the newly evaluated results
                # new_metric_results_list already filtered for target datasets
                overall_metrics = self._calculate_and_print_metrics(
                    new_metric_results_list
                )
                if (
                    isinstance(overall_metrics, dict)
                    and "metrics_calculation_error" not in overall_metrics
                ...
                final_results_to_save = {
                    "test_run_parameters": {
                        ...
                        "overlap_size": overlap_size,
                        "num_retrieved_docs": self.num_retrieved_docs,
                        "chroma_collection_used": collection.name
                        if collection
                        else "N/A",
                    },
                    ...
                    for algorithm in self.retrieval_algorithms_to_test:
                        ...
                        for file_basename in self.files_to_test:
                            for extension in self.file_extensions_to_test:
                                manual_filepath = os.path.join(
                                    MANUALS_DIRECTORY, f"{file_basename}.{extension}"
                                )
                                if not os.path.isfile(manual_filepath):
                                    logging.debug(
                                        f"Skipping, file not found: {manual_filepath}"
                                    )
                                    continue
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            shared_embedding_retriever = EmbeddingRetriever(
                model_config=embedding_model_config
            )
            logging.info(
                "--- Shared Embedding Model Initialized for Standalone Run ---"
            )
            ...
            logging.error(
                f"Error loading configuration or initializing model for pre-check: {config_ex}",
                exc_info=True,
            )
            raise
        ...
        start_rag_tests(
            config_path=config_to_test, embedding_retriever=shared_embedding_retriever
        )
