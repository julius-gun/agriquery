...
def create_f1_heatmap(
    ...
    values_col: str = "f1_score",  # Column containing the actual data values
    value_label: Optional[str] = None,  # NEW: Display name for the metric in the title
    all_indices: Optional[List[str]] = None,
    ...
    if data is None or data.empty:
        ...
        print(
            f"Warning: Input data for heatmap '{output_path}' is None or empty. Skipping plot generation."
        )
        return
    if not all(col in data.columns for col in [index_col, columns_col, values_col]):
        print(
            f"Error: DataFrame must contain columns: '{index_col}', '{columns_col}', '{values_col}'."
        )
    ...
    try:
        if DEBUG_HEATMAP:
            print("--- Heatmap Debug (create_f1_heatmap): Creating pivot table ---")
        ...
        if DEBUG_HEATMAP:
            print(
                f"--- Heatmap Debug (create_f1_heatmap): Pivot table BEFORE reindexing/sorting ---\n{pivot_df.to_string()}\n"
                + "-" * 30
            )
    ...
    if all_indices:
        if DEBUG_HEATMAP:
            print(
                f"--- Heatmap Debug (create_f1_heatmap): Ensuring heatmap index includes: {all_indices} ---"
            )
        ...
        if DEBUG_HEATMAP:
            print(
                f"--- Heatmap Debug (create_f1_heatmap): Pivot table AFTER reindexing ---\n{pivot_df.to_string()}\n"
                + "-" * 30
            )
    ...
    if sort_columns_by_value and not pivot_df.empty:
        try:
            if DEBUG_HEATMAP:
                print(
                    f"--- Heatmap Debug (create_f1_heatmap): Sorting columns by mean '{values_col}' (ascending) ---"
                )
            ...
            print(
                f"Warning: Could not sort columns by value for {output_path}. Error: {e}"
            )
            # Continue without sorted columns

    if pivot_df.isnull().all().all():
        print(
            f"Warning: Pivot table for {output_path} contains only NaN values after pivoting and reindexing. Heatmap might be blank."
        )
    ...
    if DEBUG_HEATMAP:
        print("--- Heatmap Debug (create_f1_heatmap): Generating heatmap plot ---")
    ax = sns.heatmap(
        pivot_df,
        annot=True,
        fmt=annot_fmt,
    ...
    value_display_name = value_label if value_label else values_col.replace("_", " ").title()
    default_title = f'{value_display_name} ({index_col.title()} vs {columns_col.title()})'
    if current_params:
...
def create_chunk_overlap_heatmap(
    ...
    fixed_params: Optional[
        Dict[str, Any]
    ] = None,  # Parameters held constant (for title)
    title: Optional[str] = None,
    xlabel: Optional[str] = None,
    ylabel: Optional[str] = None,
    cmap: str = "viridis",
    ...
    Args:
        ...
        output_path: The full path where the plot image will be saved.
        values_col: The column name containing the scores for the heatmap cells.
        value_label: Optional string to use as the metric name in the plot title (e.g., "F1 Score").
                     If None, the title will be derived from `values_col`.
    ...
    if data is None or data.empty:
        print(
            f"Warning: Input data for chunk/overlap heatmap '{output_path}' is None or empty. Skipping plot generation."
        )
        return
    if not all(col in data.columns for col in [index_col, columns_col, values_col]):
        print(
    ...
    try:
        if DEBUG_HEATMAP:
            print(
                "--- Heatmap Debug (create_chunk_overlap_heatmap): Creating pivot table ---"
            )
        pivot_df = pd.pivot_table(
            data,  # Use the passed (already filtered) data
            values=values_col,
        ...
        if DEBUG_HEATMAP:
            print(
                f"--- Heatmap Debug (create_chunk_overlap_heatmap): Pivot table ---\n{pivot_df.to_string()}\n"
                + "-" * 30
            )
    except KeyError as e:
        print(
            f"Error creating pivot table for {output_path}: Missing column {e}. Ensure '{index_col}', '{columns_col}', and '{values_col}' exist in the filtered data."
        )
        return
    except Exception as e:
        print(f"Error creating pivot table for {output_path}: {e}")
    ...
    if pivot_df.isnull().all().all():
        print(
            f"Warning: Pivot table for {output_path} contains only NaN values. Heatmap might be blank."
        )
    ...
    if DEBUG_HEATMAP:
        print(
            "--- Heatmap Debug (create_chunk_overlap_heatmap): Generating heatmap plot ---"
        )
    ax = sns.heatmap(
        pivot_df,
        annot=True,
    ...
    value_display_name = value_label if value_label else values_col.replace("_", " ").title()
    default_title = f'{value_display_name} ({index_col.replace("_", " ").title()} vs {columns_col.replace("_", " ").title()})'
    if fixed_params:
        param_parts = []
...
def create_model_vs_chunk_overlap_heatmap(
    ...
    fixed_params: Optional[
        Dict[str, Any]
    ] = None,  # Parameters held constant (e.g., language, algorithm)
    title: Optional[str] = None,
    xlabel: Optional[str] = None,
    ylabel: Optional[str] = None,
    cmap: str = "viridis",
    ...
    if data is None or data.empty:
        print(
            f"Warning: Input data for model vs chunk/overlap heatmap '{output_path}' is None or empty. Skipping plot generation."
        )
        return
    ...
    try:
        if DEBUG_HEATMAP:
            print(
                "--- Heatmap Debug (create_model_vs_chunk_overlap_heatmap): Creating pivot table ---"
            )
        pivot_df = pd.pivot_table(
            data,  # Use the passed (already filtered) data
            values=values_col,
        ...
        pivot_df = pivot_df.sort_index(level=[0, 1], ascending=[True, True]).sort_index(
            axis=1
        )
        if DEBUG_HEATMAP:
            print(
                f"--- Heatmap Debug (create_model_vs_chunk_overlap_heatmap): Pivot table AFTER initial sorting ---\n{pivot_df.to_string()}\n"
                + "-" * 30
            )
    except KeyError as e:
        print(
            f"Error creating pivot table for {output_path}: Missing column {e}. Ensure {required_cols} exist in the filtered data."
        )
        return
    except Exception as e:
        print(f"Error creating pivot table for {output_path}: {e}")
    ...
    if sort_columns_by_value and not pivot_df.empty:
        try:
            if DEBUG_HEATMAP:
                print(
                    f"--- Heatmap Debug (create_model_vs_chunk_overlap_heatmap): Sorting columns by mean '{values_col}' (ascending) ---"
                )
    ...
    if DEBUG_HEATMAP:
        print(
            "--- Heatmap Debug (create_model_vs_chunk_overlap_heatmap): Generating heatmap plot ---"
        )
    ax = sns.heatmap(
        pivot_df,
        annot=True,
    ...
    plt.yticks(
        rotation=0
    )  # MultiIndex labels might get long, but 0 rotation is usually fine

    # Construct title based on fixed parameters
    value_display_name = value_label if value_label else values_col.replace("_", " ").title()
    default_title = f'{value_display_name} (Model vs Chunk/Overlap)'
    if fixed_params:
        param_parts = []
        # Define preferred order or filter specific keys if needed
    ...
    y_axis_label = (
        ylabel
        or f"{index_col_chunk.replace('_', ' ').title()} / {index_col_overlap.replace('_', ' ').title()}"
    )
...
if __name__ == "__main__":
    ...
    print(
        "--- Testing Heatmap Creation (Example - Run main_visualization.py for actual use) ---"
    )
    ...
    try:
        ...
        if not os.path.exists(config_path):
            ...
            print(
                f"Warning: Config file not found at {config_path}. Using default language list for example."
            )
            ...
            if not all_languages_list:
                print(
                    "Warning: No languages found in config file. Using default list for example."
                )
                all_languages_list = ["english", "french", "german"]  # Default example
        ...
        if example_df is not None and not example_df.empty:
            ...
            test_output_dir = os.path.join(
                visualization_dir, "plots_test"
            )  # Save to a test subdir
            ...
            print(
                f"Simulating chunk/overlap heatmap generation for fixed parameters: {fixed_params_chunk}"
            )
            ...
            if not filtered_df_for_chunk_test.empty:
                ...
                test_output_path_chunk = os.path.join(
                    test_output_dir,
                    f"test_chunk_overlap_f1_heatmap_{fixed_param_str_filename_chunk}.png",
                )
            ...
            print(
                f"Simulating model vs chunk/overlap heatmap generation for fixed parameters: {fixed_params_model_chunk}"
            )
            ...
            if not filtered_df_for_model_chunk_test.empty:
                ...
                test_output_path_model_chunk = os.path.join(
                    test_output_dir,
                    f"test_model_vs_chunk_overlap_f1_heatmap_{fixed_param_str_filename_model_chunk}.png",
                )
                ...
                print(
                    f"No data found for create_model_vs_chunk_overlap_heatmap test matching fixed parameters: {fixed_params_model_chunk}."
                )
                print(
                    "Ensure your 'results' directory contains 'english' language files for the specified algorithm ('embedding') with varying models, chunk sizes, and overlap sizes."
                )

        else:
            print(
                "Could not load example data. Ensure 'results' directory exists and contains valid files."
            )
